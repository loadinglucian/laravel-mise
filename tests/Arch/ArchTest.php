...
arch('service providers extend Laravel ServiceProvider')
    ->expect(\Bigpixelrocket\LaravelBuffet\BuffetServiceProvider::class)
...
arch('tests follow best practices')
    ...
    ->ignoring('Tests\Feature\Commands\BuffetCommandTest'); // Exception for package verification tests
...
arch('source code uses Laravel facades for file operations')
    ...
    ->ignoring([
        \Bigpixelrocket\LaravelBuffet\Commands\BuffetCommand::class, // copyFile method uses file_get_contents/file_put_contents for performance
    ]);
