...
arch('service providers extend Laravel ServiceProvider')
    ->expect(\LaravelMise\MiseServiceProvider::class)
...
arch('source code uses Laravel facades for file operations')
    ...
    ->ignoring([
        \LaravelMise\Commands\MiseCommand::class, // copyFile method uses file_get_contents/file_put_contents for performance
    ]);

// Ensure concrete commands have proper structure
arch('commands have handle method')
    ->expect([
        \LaravelMise\Commands\MiseCommand::class,
        \LaravelMise\Commands\DbMigrateCommand::class,
    ])
    ->toHaveMethod('handle');
